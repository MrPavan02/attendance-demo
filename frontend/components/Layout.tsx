...
const Layout: React.FC<LayoutProps> = ({ children, user, onLogout }) => {
  return (
    <div className="min-h-screen flex flex-col bg-[#F1F6F9] text-[#14274E]">
      <header className="bg-white border-b border-[#9BA4B4] shadow-sm sticky top-0 z-50">
        <div className="container mx-auto px-6 py-4 flex justify-between items-center">
          <div className="flex items-center space-x-4">
            <div className="bg-[#14274E] p-2 rounded-lg">
          ...
          {user && (
            <div className="flex items-center space-x-6">
              <div className="hidden md:block text-right">
                <p className="text-sm font-semibold text-[#14274E]">{user.name}<br /><span className="text-[10px] font-bold text-[#394867] uppercase tracking-tighter">ID: {user.id}</span></p>
              </div>
              <button 
                onClick={onLogout}
                className="bg-[#F1F6F9] hover:bg-[#D6E4F0] text-[#394867] transition-colors px-4 py-2 rounded-lg text-xs font-bold border border-[#9BA4B4] flex items-center"
              >
      ...
      {user && (
        <footer className="bg-white border-t border-[#9BA4B4] py-3">
          <div className="container mx-auto px-6 text-center text-[#394867] text-[10px] font-bold uppercase tracking-widest">
